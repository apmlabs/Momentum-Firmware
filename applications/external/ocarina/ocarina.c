...
const uint8_t u8g2_font_open_iconic_play_1x_t[] =
    ...
    "N\15\210\24\213f\231\234\\(\66\22\0O\20\210\24/,\221\220,\221J\244\244\42\14\2P\13\206"
...
void draw_glyph(Canvas* canvas, const char* token, unsigned int x, unsigned int y) {
    if(strcmp(token, "Left") == 0) {
        canvas_draw_glyph(canvas, x, y, 0x004D);
    } else if(strcmp(token, "Right") == 0) {
        canvas_draw_glyph(canvas, x, y, 0x004E);
    } else if(strcmp(token, "Up") == 0) {
        canvas_draw_glyph(canvas, x, y, 0x004F);
    } else if(strcmp(token, "Down") == 0) {
        canvas_draw_glyph(canvas, x, y, 0x004C);
    } else if(strcmp(token, "Ok") == 0) {
        canvas_set_custom_u8g2_font(canvas, u8g2_font_open_iconic_play_1x_t);
        canvas_draw_glyph(canvas, x, y, 0x0046);
...
// Fixed: acquire speaker once at start, not on every note
void play_note(Ocarina* ocarina, float frequency) {
    if(!ocarina->speaker_acquired) {
        if(furi_hal_speaker_acquire(100)) {
            ocarina->speaker_acquired = true;
        } else {
            return; // Can't get speaker, skip
        }
    }
    furi_hal_speaker_start(frequency, 1.0f);
}

void stop_note(Ocarina* ocarina) {
    if(ocarina->speaker_acquired) {
...
void input_callback(InputEvent* input, void* ctx) {
    Ocarina* ocarina = ctx;
    furi_message_queue_put(ocarina->event_queue, input, 0); // Don't block!
}
...
Ocarina* ocarina_alloc() {
    ...
    instance->speaker_acquired = false;
    memset(instance->key_down, 0, sizeof(instance->key_down));
    return instance;
}
    ...
    while(processing) {
        ...
        if(status == FuriStatusOk) {
            if(event.type == InputTypePress || event.type == InputTypeLong) {
                ...
                if(event.key < INPUT_KEY_COUNT) {
                    ocarina->key_down[event.key] = false;
                }
                // Only stop if no keys are held
                bool any_held = false;
                for(int i = 0; i < INPUT_KEY_COUNT; i++) {
                    if(ocarina->key_down[i]) {
                        any_held = true;
                        break;
                    }
                }
