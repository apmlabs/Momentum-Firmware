#include "dab_timer.h"
/* dab_timer_icons.h removed - no icon assets */
...
void dab_timer_render_binary_face(
    ...
    if(value >= 32) {
        canvas_draw_icon(canvas, h_loc_start, height, NULL);
        value = value - 32;
    } else {
        canvas_draw_icon(canvas, h_loc_start, height, NULL);
    }
    if(value >= 16) {
        canvas_draw_icon(canvas, h_loc_start + (14 * 1), height, NULL);
        value = value - 16;
    } else {
        canvas_draw_icon(canvas, h_loc_start + (14 * 1), height, NULL);
    }
    if(value >= 8) {
        canvas_draw_icon(canvas, h_loc_start + (14 * 2), height, NULL);
        value = value - 8;
    } else {
        canvas_draw_icon(canvas, h_loc_start + (14 * 2), height, NULL);
    }
    if(value >= 4) {
        canvas_draw_icon(canvas, h_loc_start + (14 * 3), height, NULL);
        value = value - 4;
    } else {
        canvas_draw_icon(canvas, h_loc_start + (14 * 3), height, NULL);
    }
    if(value >= 2) {
        canvas_draw_icon(canvas, h_loc_start + (14 * 4), height, NULL);
        value = value - 2;
    } else {
        canvas_draw_icon(canvas, h_loc_start + (14 * 4), height, NULL);
    }
    if(value >= 1) {
        canvas_draw_icon(canvas, h_loc_start + (14 * 5), height, NULL);
        value = value - 1;
    } else {
        canvas_draw_icon(canvas, h_loc_start + (14 * 5), height, NULL);
    }
...
static void dab_timer_render_callback(Canvas* const canvas, void* ctx) {
    ...
    if(plugin_state->faceType == FaceStylePwn || plugin_state->faceType == FaceStylePwnInverted) {
        if(plugin_state->faceType == FaceStylePwnInverted) {
            canvas_draw_icon(canvas, 0, 0, NULL);
            if(timer_start_timestamp != 0) {
        ...
        if(plugin_state->w_test && timer_start_timestamp != 0) {
            ...
            canvas_draw_icon(canvas, 1, 32, NULL);
        }
