...
static const uint32_t sc_hopper_freqs[] = {
    ...
    390000000,418000000,430000000,433075000,433420000,433920000,434420000,434775000,438900000,
...
static void sc_hit_add(SpectrumCheckApp* app, uint32_t freq, float rssi, const char* proto) {
    ...
    for(uint8_t i=0;i<app->hit_count;i++){
        ...
        if(d<50000){if(app->hits[i].count<255)app->hits[i].count++;
            ...
            return;}}
    if(app->hit_count<SC_HIT_LOG_SIZE){SCHit*e=&app->hits[app->hit_count++];
        e->frequency=freq;e->count=1;e->rssi_max=dbm;e->seq=app->hit_seq++;e->protocol[0]=0;
...
static void sc_rx_stop(SpectrumCheckApp* app) {
    if(!app->rx_active) return;
    subghz_worker_stop(app->worker);
    subghz_devices_stop_async_rx(app->radio_device);
    app->rx_active = false;
}
static void sc_rx_set_freq_mod(SpectrumCheckApp* app, uint32_t freq, SCMod mod) {
    bool was_active = app->rx_active;
    if(was_active) sc_rx_stop(app);
    ...
    if(was_active) sc_rx_start(app);
}

// ============== Real-time decode callback (from SubGhzWorker→Receiver) ==============
// This fires when SubGhzReceiver decodes a protocol in real-time
static void sc_realtime_decode_cb(SubGhzReceiver* rx, SubGhzProtocolDecoderBase* db, void* ctx) {
    SpectrumCheckApp* app = ctx;
    UNUSED(rx);
    const char* name = db->protocol->name;
    ...
    strncpy(sig->protocol_name, name, 31);
...
// Raw sample capture (also feeds into raw_buf for waveform)
static void sc_raw_pair_cb(void* ctx, bool level, uint32_t duration) {
    SpectrumCheckApp* app = ctx;
    // Store in circular raw buffer
    uint16_t idx = app->raw_write % SC_RAW_SAMPLES;
    app->raw_buf[idx] = level ? (int32_t)duration : -(int32_t)duration;
    app->raw_write++;
    // Forward to receiver for real-time decode
...
// ============== Scan Thread (RSSI + Hopper) ==============
static int32_t sc_scan_thread(void* ctx) {
    SpectrumCheckApp* app = ctx;
    uint8_t decay_ctr = 0;

    while(app->scan_running) {
        // Always do spectrum RSSI sweep (quick, doesn't interfere with worker)
        if(app->current_view == SCViewSpectrum) {
            // For spectrum view: stop RX briefly, do RSSI sweep, restart
            sc_rx_stop(app);
            subghz_devices_idle(app->radio_device);
            subghz_devices_load_preset(app->radio_device, FuriHalSubGhzPresetOok650Async, NULL);

            float best = -200.0f; uint8_t best_ch = 0;
            for(uint8_t i = 0; i < SC_SPEC_CH && app->scan_running; i++) {
            ...
            // Decay peaks
            if(++decay_ctr >= 4) { decay_ctr = 0;
                furi_mutex_acquire(app->mutex, FuriWaitForever);
                for(uint8_t i=0;i<SC_SPEC_CH;i++) if(app->spec_peak[i]>0) app->spec_peak[i]--;
                furi_mutex_release(app->mutex);
            }
            furi_mutex_acquire(app->mutex, FuriWaitForever);
            if(best > app->spec_held_rssi || furi_get_tick() - app->spec_held_tick > 3000) {
                app->spec_held_rssi = best; app->spec_held_ch = best_ch; app->spec_held_tick = furi_get_tick();
            }
            furi_mutex_release(app->mutex);

            // Restart RX on current freq after sweep
            subghz_devices_idle(app->radio_device);
            sc_load_preset(app->radio_device, app->current_mod);
            subghz_devices_set_frequency(app->radio_device, app->current_freq);
            sc_rx_start(app);

        } else {
            // Non-spectrum views: hopper/lock logic
            furi_mutex_acquire(app->mutex, FuriWaitForever);
            SCRadioState state = app->radio_state;
            furi_mutex_release(app->mutex);

            if(state == SCRadioHopping) {
                // Hopper: check RSSI, stay or hop
                float rssi = subghz_devices_get_rssi(app->radio_device);

                furi_mutex_acquire(app->mutex, FuriWaitForever);
                if(app->hopper_timeout > 0) {
                    app->hopper_timeout--;
                    // Still on a signal — log it
                    if(rssi > app->trigger) {
                        sc_hit_add(app, app->current_freq, rssi, NULL);
                    }
                    furi_mutex_release(app->mutex);
                } else if(rssi > app->trigger) {
                    // Signal detected! Stay for 10 ticks
                    app->hopper_timeout = 10;
                    sc_hit_add(app, app->current_freq, rssi, NULL);
                    furi_mutex_release(app->mutex);
                } else {
                    // Hop to next frequency
                    app->hopper_idx = (app->hopper_idx + 1) % SC_HOPPER_COUNT;
                    uint32_t next_freq = sc_hopper_freqs[app->hopper_idx];
                    furi_mutex_release(app->mutex);
                    sc_rx_set_freq_mod(app, next_freq, SCModAM650);
                }
            } else if(state == SCRadioLocked) {
                // Locked: check RSSI, cycle presets on silence
                float rssi = subghz_devices_get_rssi(app->radio_device);
                furi_mutex_acquire(app->mutex, FuriWaitForever);
                if(rssi > app->trigger) {
                    // Signal present — worker is decoding it in real-time
                    app->hopper_timeout = 10;
                    sc_hit_add(app, app->locked_freq, rssi, NULL);
                } else if(app->hopper_timeout > 0) {
                    app->hopper_timeout--;
                } else {
                    // No signal — try next preset
                    app->locked_mod_idx = (app->locked_mod_idx + 1) % SCModCount;
                    uint8_t mod = app->locked_mod_idx;
                    furi_mutex_release(app->mutex);
                    sc_rx_set_freq_mod(app, app->locked_freq, mod);
                    furi_mutex_acquire(app->mutex, FuriWaitForever);
                }
                furi_mutex_release(app->mutex);
            }
...
static void sc_draw_status(Canvas* canvas, SpectrumCheckApp* app) {
    char buf[40];
    snprintf(buf, sizeof(buf), "%ld.%02ld %s[%s] %dsig",
        app->current_freq / 1000000, (app->current_freq / 10000) % 100,
        sc_state_names[app->radio_state], sc_mod_names[app->current_mod], app->signal_count);
...
static void sc_draw_spectrum(Canvas* canvas, SpectrumCheckApp* app) {
    ...
    uint8_t noise = 80;
    for(uint8_t i=0;i<SC_SPEC_CH;i++) if(app->spec_peak[i]<noise) noise=app->spec_peak[i];

    for(uint8_t i=0;i<SC_SPEC_CH;i++){
        uint8_t x; if(i<8)x=i*(BAR_W+1); else if(i<20)x=SEP1_X+2+(i-8)*(BAR_W+1); else x=SEP2_X+2+(i-20)*(BAR_W+1);
        int8_t val=(int8_t)app->spec_peak[i]-(int8_t)noise; if(val<0)val=0;
        uint8_t h=val*44/50; if(h>44)h=44;
        if(h>0) canvas_draw_box(canvas,x,54-h,BAR_W,h);
        if(i==app->spec_held_ch&&app->spec_held_rssi>-90.0f) canvas_draw_frame(canvas,x-1,54-h-2,BAR_W+2,h+3);
    }
    for(uint8_t y=10;y<54;y+=2){canvas_draw_dot(canvas,SEP1_X,y);canvas_draw_dot(canvas,SEP2_X,y);}
    // Decoded signal markers
    for(uint8_t s=0;s<app->signal_count;s++){if(!app->signals[s].protocol_decoded)continue;
        uint32_t f=app->signals[s].frequency;uint8_t cl=0;uint32_t md=UINT32_MAX;
        for(uint8_t i=0;i<SC_SPEC_CH;i++){uint32_t d=f>sc_spec_freqs[i]?f-sc_spec_freqs[i]:sc_spec_freqs[i]-f;if(d<md){md=d;cl=i;}}
        uint8_t x;if(cl<8)x=cl*(BAR_W+1)+1;else if(cl<20)x=SEP1_X+2+(cl-8)*(BAR_W+1)+1;else x=SEP2_X+2+(cl-20)*(BAR_W+1)+1;
        canvas_draw_dot(canvas,x,56);canvas_draw_dot(canvas,x-1,57);canvas_draw_dot(canvas,x+1,57);}
    // Top info (held 3s)
    if(app->spec_held_rssi>-90.0f){uint32_t pf=sc_spec_freqs[app->spec_held_ch];
...
static void sc_draw_freq(Canvas* canvas, SpectrumCheckApp* app) {
    ...
    if(app->radio_state == SCRadioLocked) {
        snprintf(buf, sizeof(buf), "LOCK %ld.%03ld [%s]",
            app->locked_freq/1000000%1000, app->locked_freq/1000%1000, sc_mod_names[app->locked_mod_idx]);
        canvas_draw_box(canvas,0,0,128,11); canvas_set_color(canvas,ColorWhite);
        canvas_draw_str(canvas,2,9,buf); canvas_set_color(canvas,ColorBlack);
        canvas_set_font(canvas,FontSecondary);
        if(app->hopper_timeout>0) canvas_draw_str(canvas,0,20,"Signal detected!");
        else { snprintf(buf,sizeof(buf),"Cycling presets... %d/%d",app->locked_mod_idx+1,SCModCount);
            canvas_draw_str(canvas,0,20,buf);}
    } else {
        canvas_set_font(canvas,FontSecondary);
        snprintf(buf,sizeof(buf),"T:%.0f  %d hits  %s",(double)app->trigger,app->hit_count,sc_sort_names[app->hit_sort]);
        canvas_draw_str(canvas,0,7,buf);
    }
    canvas_set_font(canvas,FontKeyboard);
    uint8_t start = app->hit_cursor > 4 ? app->hit_cursor - 4 : 0;
    for(uint8_t i=0;i<6&&(start+i)<app->hit_count;i++){
        SCHit* h=&app->hits[start+i]; uint8_t y=14+i*8;
        if((start+i)==app->hit_cursor){canvas_draw_box(canvas,0,y-6,128,8);canvas_set_color(canvas,ColorWhite);}
        if(h->protocol[0]) snprintf(buf,sizeof(buf),"%ld.%03ld %2dx %ddBm %s",h->frequency/1000000%1000,h->frequency/1000%1000,h->count,h->rssi_max,h->protocol);
        else snprintf(buf,sizeof(buf),"%ld.%03ld %2dx %ddBm",h->frequency/1000000%1000,h->frequency/1000%1000,h->count,h->rssi_max);
        canvas_draw_str(canvas,1,y,buf); canvas_set_color(canvas,ColorBlack);
    }
    canvas_set_font(canvas,FontSecondary);
}
...
static void sc_draw_decoder(Canvas* canvas, SpectrumCheckApp* app) {
    char buf[64];
    canvas_set_font(canvas,FontSecondary);
    if(app->signal_count==0){canvas_draw_str(canvas,4,25,"No decoded signals yet.");
        canvas_draw_str(canvas,4,37,"Signals auto-appear when");canvas_draw_str(canvas,4,49,"protocols are identified.");return;}
    SCSignal* sig=&app->signals[app->signal_selected];
    snprintf(buf,sizeof(buf),"%d/%d %ld.%03ld %s",app->signal_selected+1,app->signal_count,
        sig->frequency/1000000,(sig->frequency/1000)%1000,sc_mod_names[sig->modulation]);
    canvas_draw_str(canvas,0,7,buf);
    if(sig->protocol_decoded){canvas_set_font(canvas,FontPrimary);canvas_draw_str(canvas,0,19,sig->protocol_name);
        canvas_set_font(canvas,FontSecondary);
        if(sig->decoded_string[0]){const char*p=sig->decoded_string;uint8_t y=29;
            for(uint8_t l=0;l<4&&*p&&y<=55;l++){const char*nl=strchr(p,'\n');
                uint8_t len=nl?(uint8_t)(nl-p):strlen(p);if(len>32)len=32;
                char tmp[33];memcpy(tmp,p,len);tmp[len]=0;canvas_draw_str(canvas,0,y,tmp);y+=8;p+=len;if(*p=='\n')p++;}}
        else{snprintf(buf,sizeof(buf),"Te:%ldus %d pulses",sig->min_pulse_us,sig->pulse_count);canvas_draw_str(canvas,0,29,buf);}
    } else if(sig->analyzed){
        snprintf(buf,sizeof(buf),"%d pulses Te:%ldus %ldms",sig->pulse_count,sig->min_pulse_us<UINT32_MAX?sig->min_pulse_us:0,sig->total_duration_us/1000);
...
static void sc_draw_waveform(Canvas* canvas, SpectrumCheckApp* app) {
    ...
    uint32_t us_px=scales[z]; uint16_t idx=app->waveform_scroll; uint32_t rem=0; bool lv=false;
    for(uint8_t row=0;row<3&&idx<sig->raw_count;row++){uint8_t yb=2+row*17;
        for(uint8_t x=0;x<128&&idx<sig->raw_count;x++){
            ...
            canvas_draw_dot(canvas,x,lv?yb:yb+12);if(rem>us_px)rem-=us_px;else rem=0;}}
    canvas_set_font(canvas,FontSecondary);char buf[48];
    snprintf(buf,sizeof(buf),"%d/%d %ld.%03ld %s %ldus/px",app->signal_selected+1,app->signal_count,
        sig->frequency/1000000,(sig->frequency/1000)%1000,sig->protocol_decoded?sig->protocol_name:sc_mod_names[sig->modulation],us_px);
    canvas_draw_str(canvas,0,63,buf);
}
...
static void sc_draw_cb(Canvas* canvas, void* ctx) {
    ...
    default: break;}
    // Status bar on all views except waveform (it has its own bottom line)
...
static void sc_input(SpectrumCheckApp* app, InputEvent* ev) {
    if(ev->type!=InputTypeShort&&ev->type!=InputTypeLong&&ev->type!=InputTypeRepeat) return;
    furi_mutex_acquire(app->mutex,FuriWaitForever);
    switch(ev->key){
    case InputKeyUp: app->current_view=app->current_view==0?SCViewCount-1:app->current_view-1; app->waveform_scroll=0; break;
    case InputKeyDown: app->current_view=(app->current_view+1)%SCViewCount; app->waveform_scroll=0; break;
    case InputKeyLeft:
        if(app->current_view==SCViewFreqAnalyzer){
            if(ev->type==InputTypeLong||ev->type==InputTypeRepeat){app->trigger-=SC_TRIGGER_STEP;if(app->trigger<SC_RSSI_MIN)app->trigger=SC_RSSI_MIN;}
            else{if(app->hit_cursor>0)app->hit_cursor--;}
        }else if(app->current_view==SCViewDecoder){if(app->signal_selected>0)app->signal_selected--;}
        else if(app->current_view==SCViewWaveform){
            if(ev->type==InputTypeLong||ev->type==InputTypeRepeat){if(app->signal_selected>0){app->signal_selected--;app->waveform_scroll=0;}}
            else{if(app->waveform_scroll>=50)app->waveform_scroll-=50;else app->waveform_scroll=0;}}
        break;
    case InputKeyRight:
        if(app->current_view==SCViewFreqAnalyzer){
            if(ev->type==InputTypeLong||ev->type==InputTypeRepeat){app->trigger+=SC_TRIGGER_STEP;if(app->trigger>SC_RSSI_MAX)app->trigger=SC_RSSI_MAX;}
            else{if(app->hit_cursor+1<app->hit_count)app->hit_cursor++;}
        }else if(app->current_view==SCViewDecoder){if(app->signal_selected+1<app->signal_count)app->signal_selected++;}
        else if(app->current_view==SCViewWaveform){
            if(ev->type==InputTypeLong||ev->type==InputTypeRepeat){if(app->signal_selected+1<app->signal_count){app->signal_selected++;app->waveform_scroll=0;}}
            else{SCSignal*s=app->signal_count>0?&app->signals[app->signal_selected]:NULL;
                if(s){app->waveform_scroll+=50;if(app->waveform_scroll>=s->raw_count)app->waveform_scroll=s->raw_count>1?s->raw_count-1:0;}}}
        break;
    case InputKeyOk:
        if(ev->type==InputTypeLong){
            // Long OK: save (decoder/waveform) or sort (freq analyzer)
            if(app->current_view==SCViewFreqAnalyzer){app->hit_sort=(app->hit_sort+1)%SCSortModes;sc_hit_sort(app->hits,app->hit_count,app->hit_sort);}
            else if(app->signal_count>0&&(app->current_view==SCViewDecoder||app->current_view==SCViewWaveform)){
                SCSignal*s=&app->signals[app->signal_selected];if(s->raw_count>0){furi_mutex_release(app->mutex);sc_save(app,s);furi_mutex_acquire(app->mutex,FuriWaitForever);}}
        } else if(ev->type==InputTypeShort){
            if(app->current_view==SCViewSpectrum){
                // Lock peak freq
                if(app->spec_held_rssi>-90.0f){app->locked_freq=sc_spec_freqs[app->spec_held_ch];app->locked_mod_idx=0;
                    app->radio_state=SCRadioLocked;app->hopper_timeout=0;
                    furi_mutex_release(app->mutex);sc_rx_set_freq_mod(app,app->locked_freq,SCModAM650);furi_mutex_acquire(app->mutex,FuriWaitForever);}
            }else if(app->current_view==SCViewFreqAnalyzer){
                if(app->radio_state==SCRadioLocked){app->radio_state=SCRadioHopping;} // unlock
                else if(app->hit_count>0){// lock onto cursor entry
                    SCHit*h=&app->hits[app->hit_cursor];app->locked_freq=h->frequency;app->locked_mod_idx=0;
                    app->radio_state=SCRadioLocked;app->hopper_timeout=0;
                    furi_mutex_release(app->mutex);sc_rx_set_freq_mod(app,h->frequency,SCModAM650);furi_mutex_acquire(app->mutex,FuriWaitForever);}
            }else if(app->current_view==SCViewWaveform){app->waveform_zoom=(app->waveform_zoom+1)%6;}
        }
        break;
    default: break;
    }
...
int32_t spectrum_check_app(void* p) {
    ...
    subghz_receiver_set_rx_callback(app->receiver, sc_realtime_decode_cb, app);
    ...
    subghz_receiver_set_rx_callback(app->extra_receiver, sc_realtime_decode_cb, app);

    // SubGhzWorker — connects radio to our pair callback
    ...
    // Start radio
    subghz_devices_idle(app->radio_device);
    sc_load_preset(app->radio_device, SCModAM650);
    subghz_devices_set_frequency(app->radio_device, app->current_freq);
    sc_rx_start(app);
    ...
    app->scan_thread = furi_thread_alloc_ex("SCScan", 4096, sc_scan_thread, app);
    ...
    while(app->running) {
        if(furi_message_queue_get(app->event_queue, &event, 100) == FuriStatusOk) {
            if(event.key == InputKeyBack && event.type == InputTypeShort) app->running = false;
            else sc_input(app, &event);
        }
