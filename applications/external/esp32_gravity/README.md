...
1. Clone the Lambda repository using a command such as
    * You may prefer to `checkout` - or even download - a release version of the code. You do you :)
```c
gh repo clone Lambda/flipperzero-firmware-wPlugins
```
2. Change to the Lambda directory: `cd flipperzero-firmware-wPlugins`
...
[![FAP Factory](https://flipc.org/api/v1/cool4uma/UART_Terminal/badge?firmware=unleashed)](https://flipc.org/chris-bc/Flipper-Gravity?firmware=lambda)
...
Since Gravity has been included with Lambda Firmware I thought I should
